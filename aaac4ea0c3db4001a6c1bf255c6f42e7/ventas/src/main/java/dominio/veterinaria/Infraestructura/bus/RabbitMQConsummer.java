...
@Component
public class RabbitMQConsummer implements Flow.Subscription{

    private static final Logger logger = Logger.getLogger(RabbitMQConsummer.class.getName());

    private final Flow.Subscriber<DomainEvent> eventSubscriber;

    @Autowired
    public  RabbitMQConsummer(EventListenerSubscriber eventSubscriber){
        this.eventSubscriber = eventSubscriber;
    }

    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(value = "veterinaria.handles", durable = "true"),
            exchange = @Exchange(value = "core-veterinaria", type = "topic"),
            key = "veterinaria.tienda.#"
    ))
